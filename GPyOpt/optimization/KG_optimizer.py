...
class KGOptimizer(object):
    ...
    def __init__(self, space, optimizer='lbfgs', inner_optimizer='lbfgs', n_starting=12, n_anchor=12, **kwargs):
    ...
    def optimize2(self, f=None, df=None, f_df=None, duplicate_manager=None, f_aux=None):
        """
        ...
        anchor_points_generator = ObjectiveAnchorPointsGenerator(self.space, latin_design_type, f, self.n_starting)
        ...
        if parallel:
            ...
            while i < self.n_anchor:
                points_to_optimize = anchor_points[i:i+3,:]
                points_to_optimize = np.vstack((points_to_optimize, initial_design('random', self.space, 1)))       
        ...
        if  fx_min_anchor < fx_min + 1e-3:
            anchor_points_generator = ObjectiveAnchorPointsGenerator(self.space, random_design_type, f, 200)
            ...
            fx_min = anchor_points_values[0]
            if fx_min_anchor < fx_min:
                x_min = x_min_anchor
                fx_min = fx_min_anchor
                print('anchor point was best found')
            if False:
                self.optimizer = choose_optimizer('CMA', self.context_manager.noncontext_bounds)
                if parallel:
                    points_to_optimize = anchor_points[:n_cores,:]
                    optimized_points = pool.map(self._parallel_optimization_wrapper, points_to_optimize)
                else:
                    optimized_points = apply_optimizer(self.optimizer, x_min_anchor, f=f, df=None, f_df=f_df, duplicate_manager=duplicate_manager, context_manager=self.context_manager, space = self.space)
        print('min and min value')
        print(x_min)
        print(fx_min)
        return x_min, fx_min
    ...
    def optimize(self, f=None, df=None, f_df=None, duplicate_manager=None, f_aux=None):
        ...
        if fx_min_anchor < fx_min + 1e-3:
            ...
            aux_model = GPyOpt2.models.GPModel_MCMC(exact_feval=True)
